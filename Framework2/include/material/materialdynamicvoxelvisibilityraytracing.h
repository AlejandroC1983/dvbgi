...
#ifndef _MATERIALDYNAMICVOXELVISIBILITYRAYTRACING_H_
#define _MATERIALDYNAMICVOXELVISIBILITYRAYTRACING_H_
...
class MaterialDynamicVoxelVisibilityRayTracing : public Material
{
...
protected:
	...
	* @return nothing */
	MaterialDynamicVoxelVisibilityRayTracing(string &&name) : Material(move(name), move(string("MaterialDynamicVoxelVisibilityRayTracing")))
...
public:
	...
	* @return true if the shader was loaded successfully, and false otherwise */
	...
	{
		size_t shaderSize;
		void* rayGenShaderCode  = InputOutput::readFile("../data/vulkanshaders/voxelvisibilityraytracedynamicraygeneration.rgen", &shaderSize);
		void* rayMissShaderCode = InputOutput::readFile("../data/vulkanshaders/voxelvisibilityraytracedynamicraymiss.rmiss",      &shaderSize);
		void* rayHitShaderCode  = InputOutput::readFile("../data/vulkanshaders/voxelvisibilityraytracedynamicrayhit.rchit",       &shaderSize);
		...
		m_shaderResourceName = "voxelvisibilityraytracedynamic";
	...
	* @return nothing */
	void exposeResources()
	{
		...
		assignShaderStorageBuffer(move(string("dynamicVoxelVisibilityDebugBuffer")),  move(string("dynamicVoxelVisibilityDebugBuffer")),  VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		...
		assignAccelerationStructure(move(string("dynamicraytracedaccelerationstructure")), move(string("dynamicraytracedaccelerationstructure")), VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR);
		...
		// TODO: Fix setting m_vectorNode in material manager class when doing the instance and passing the vector with the dynamic scene elements
		AccelerationStructure * dynamicraytracedaccelerationstructure = accelerationStructureM->getElement(move(string("dynamicraytracedaccelerationstructure")));
		m_vectorNode = dynamicraytracedaccelerationstructure->getVectorNode();
		...
		Buffer* temp = bufferM->getElement(move(string("sceneDescriptorDynamicBuffer")));
		...
		assignShaderStorageBuffer(move(string("sceneDescriptorDynamicBuffer")), move(string("sceneDescriptorDynamicBuffer")), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
	}
...
protected:
	...
	vec4          m_lightPosition;             //!< Light position
...
#endif _MATERIALDYNAMICVOXELVISIBILITYRAYTRACING_H_
