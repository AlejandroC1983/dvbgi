...
#ifndef _MATERIALVOXELFACEINDEXSTREAMCOMPACTION_H_
#define _MATERIALVOXELFACEINDEXSTREAMCOMPACTION_H_
...
class MaterialVoxelFaceIndexStreamCompaction : public Material
{
...
protected:
	...
	* @return nothing */
	MaterialVoxelFaceIndexStreamCompaction(string &&name) : Material(move(name), move(string("MaterialVoxelFaceIndexStreamCompaction")))
		, m_localWorkGroupsXDimension(0)
		, m_localWorkGroupsYDimension(0)
		, m_numThreadExecuted(0)
		, m_numberElementPerThread(0)
	{
...
public:
	...
	* @return true if the shader was loaded successfully, and false otherwise */
	bool loadShader()
	{
		...
		void* shaderFirstPartCode  = InputOutput::readFile("../data/vulkanshaders/voxelfaceindexstreamcompactionfirstpart.comp",  &sizeFirstPart);
		void* shaderSecondPartCode = InputOutput::readFile("../data/vulkanshaders/voxelfaceindexstreamcompactionsecondpart.comp", &sizeSecondPart);
		...
		m_shaderResourceName = "voxelfaceindexstreamcompaction";
	...
	* @return nothing */
	void exposeResources()
	{
		...
		exposeStructField(ResourceInternalType::RIT_UNSIGNED_INT, (void*)(&m_numberElementPerThread),    move(string("myMaterialData")), move(string("numberElementPerThread")));
		...
		assignShaderStorageBuffer(move(string("voxelVisibilityNumberBuffer")),        move(string("voxelVisibilityNumberBuffer")),        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		assignShaderStorageBuffer(move(string("voxelVoxelFaceIndexDebugBuffer")),     move(string("voxelVoxelFaceIndexDebugBuffer")),     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		assignShaderStorageBuffer(move(string("voxelVisibilityDynamicNumberBuffer")), move(string("voxelVisibilityDynamicNumberBuffer")), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
	}
	...
	SET(uint, m_numberElementPerThread, NumberElementPerThread)
	GETCOPY_SET(uint, m_numOccupiedVoxel, NumOccupiedVoxel)
...
protected:
	...
	uint   m_numberElementPerThread;     //!< Number of elements processed by each thread
	uint   m_numOccupiedVoxel;           //!< Number of occupied voxels after voxelization process
};

/////////////////////////////////////////////////////////////////////////////////////////////

#endif _MATERIALVOXELFACEINDEXSTREAMCOMPACTION_H_
