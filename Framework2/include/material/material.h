...
public:
	...
	* @return true if a texture sampler was found in the shader resources, and false otherwise */
	bool assignTextureToSampler(string&& textureSamplerName, string&& textureResourceName, VkDescriptorType descriptorType, VkFilter filter);
	...
	* @param filter              [in] filtering option, if value is VkFilter::VK_FILTER_MAX_ENUM it will be ignored and the sampler associated to this image will not have the value updated
	* @return true if an image sampler was found in the shader resources, and false otherwise */
	bool assignImageToSampler(string&& imageSamplerName, string&& textureResourceName, VkDescriptorType descriptorType, VkFilter filter);

	/** Assigns the combined image sampler given as parameter to the combined image sampler whose name is given as parameter, that needs to be present in the
	* shader this material is linked with
	* @param combinedImageSamplerName  [in] name of the combined image sampler to use in the shader
	* @param vectorTextureResourceName [in] vector with the names of the texture resources used for the image combined sampler
	* @return true if the combined image sampler was found in the shader resources, and false otherwise */
	bool assignCombinedImageSampler(string&& combinedImageSamplerName, vector<string>&& vectorTextureResourceName);
	...
	* @param bufferResourceName [in] name of the buffer resource to assign to the ssbo with name bufferResourceName
	* @param descriptorType     [in] flags for the descriptor set to build for this shader storage bufer
	* @return true shader storage buffer data was found in the shader resources, and false otherwise */
	...
	/** Assigns the buffers given as parameter in vectorStorageBufferName to the shader storage buffer data whose name is given
	* as parameter, that needs to be present in the shader this material is linked with
	* @param storageBufferName        [in] shader storage buffer to link with a shader storage buffer data with names vectorBufferResourceName
	* @param vectorBufferResourceName [in] vector with the names of the buffer resources to assign to the ssbo with name storageBufferName
	* @param descriptorType           [in] flags for the descriptor set to build for this shader storage bufer
	* @return true shader storage buffer data was found in the shader resources, and false otherwise */
	bool assignShaderStorageBuffer(string&& storageBufferName, vector<string>&& vectorBufferResourceName, VkDescriptorType descriptorType);

	/** Assigns the buffers given as parameter in vectorStorageBufferName to the shader storage buffer data whose name is given
	* as parameter, that needs to be present in the shader this material is linked with
	* @param accelerationStructureName         [in] acceleration structure to link with an acceleration strcuture data with name accelerationStructureResourceName
	* @param accelerationStructureResourceName [in] vector with the name of the acceleration structure resource to assign to the acceleration structure in the shader with name storageBufferName
	* @param descriptorType                    [in] flags for the descriptor set to build for this acceleration structure
	* @return true acceleration structure data was found in the shader resources, and false otherwise */
	bool assignAccelerationStructure(string&& accelerationStructureName, string&& accelerationStructureResourceName, VkDescriptorType descriptorType);
	...
	* @return nothing */
	...
	/** Clear the vectors with exposed field information (m_vectorExposedStructField, m_vectorDirtyExposedStructField,
	* m_vectorPushConstantExposedStructField and m_vectorPushConstantDirtyExposedStructField)
	* @return nothing */
	void clearExposedFieldData();
...
protected:
	...
	* @return number of dynamic uniform buffers used by the material */
	...
	bool                          m_isCompute;                                 //!< True if the material has a compute shader assigned instead of the rasterization pipeline shader (default)
	bool                          m_isRayTracing;                              //!< True if the material has a ray tracing shader assigned instead of the rasterization pipeline shader (default)
