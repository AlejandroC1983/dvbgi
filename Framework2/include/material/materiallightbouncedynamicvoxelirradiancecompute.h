...
#ifndef _MATERIALLIGHTBOUNCEDYNAMICVOXELIRRADIANCECOMPUTE_H_
#define _MATERIALLIGHTBOUNCEDYNAMICVOXELIRRADIANCECOMPUTE_H_
...
class MaterialLightBounceDynamicVoxelIrradianceCompute: public Material
{
...
protected:
	...
	* @return nothing */
	MaterialLightBounceDynamicVoxelIrradianceCompute(string &&name) : Material(move(name), move(string("MaterialLightBounceDynamicVoxelIrradianceCompute")))
		, m_numberElementToProcess(0)
		...
		, m_numberThreadPerElement(0)
	{
		...
		m_numElementPerLocalWorkgroupThread = 2;
...
public:
	...
	* @return true if the shader was loaded successfully, and false otherwise */
	bool loadShader()
	{
		...
		void* shaderFirstPartCode  = InputOutput::readFile("../data/vulkanshaders/lightbouncedynamicvoxelirradiancefirstpart.comp",  &sizeFirstPart);
		void* shaderSecondPartCode = InputOutput::readFile("../data/vulkanshaders/lightbouncedynamicvoxelirradiancesecondpart.comp", &sizeSecondPart);
		...
		m_shaderResourceName = "lightbouncedynamicvoxelirradiancecompute";
	...
	* @return nothing */
	void exposeResources()
	{
		...
		assignShaderStorageBuffer(move(string("lightBounceDynamicVoxelIrradianceDebugBuffer")), move(string("lightBounceDynamicVoxelIrradianceDebugBuffer")), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
...
protected:
	...
	vec4   m_sceneMin;		                    //!< Minimum value of the scene's aabb in xyz coordinates, number of occupied voxels
...
#endif _MATERIALLIGHTBOUNCEDYNAMICVOXELIRRADIANCECOMPUTE_H_
