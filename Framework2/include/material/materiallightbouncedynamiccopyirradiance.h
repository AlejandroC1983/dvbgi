...
#ifndef _MATERIALLIGHTBOUNCEDYNAMICCOPYIRRADIANCE_H_
#define _MATERIALLIGHTBOUNCEDYNAMICCOPYIRRADIANCE_H_
...
class MaterialLightBounceDynamicCopyIrradiance: public Material
{
...
protected:
	...
	* @return nothing */
	MaterialLightBounceDynamicCopyIrradiance(string &&name) : Material(move(name), move(string("MaterialLightBounceDynamicCopyIrradiance")))
		...
		, m_pushConstantIsStaticVoxel(vec4(0.0f))
		, m_numThreadPerLocalWorkgroup(64)
		, m_numElementPerLocalWorkgroupThread(6)
	{
...
public:
	...
	* @return true if the shader was loaded successfully, and false otherwise */
	bool loadShader()
	{
		...
		void* shaderFirstPartCode  = InputOutput::readFile("../data/vulkanshaders/lightbouncedynamiccopyirradiancefirstpart.comp", &sizeFirstPart);
		void* shaderSecondPartCode = InputOutput::readFile("../data/vulkanshaders/lightbouncedynamiccopyirradiancesecondpart.comp", &sizeSecondPart);
		...
		m_shaderResourceName = "lightbouncedynamiccopyirradiance";
	...
	* @return nothing */
	void exposeResources()
	{
		...
		assignShaderStorageBuffer(move(string("dynamicCopyIrradianceDebugBuffer")),          move(string("dynamicCopyIrradianceDebugBuffer")),          VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
	...
	SET(vec4, m_pushConstantIsStaticVoxel, PushConstantIsStaticVoxel)
...
protected:
	...
	vec4   m_pushConstantIsStaticVoxel;         //!< Push constant to know whether the current dispatch is being used for static or dynamic voxels (.x field)
...
#endif _MATERIALLIGHTBOUNCEDYNAMICCOPYIRRADIANCE_H_
