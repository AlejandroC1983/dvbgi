										...
										PushConstant*                                 pushConstant)
{
	...
	// NOTE: The destructor of spirv_cross::CompilerGLSL is crashing in one unordered_set for some shaders, pending to debug
	//       why / update the library. In the meantime and just to allow the application to work, using instance and not deallocating,
	//       giving place to a memory leak.
	spirv_cross::CompilerGLSL* glsl        = new spirv_cross::CompilerGLSL(move(shaderBinary));
	...
	ResourceInternalType resourceInternalType;
	bool integerFormat;
	for (auto &resource : resources.sampled_images)
	{
		...
		spirv_cross::SPIRType& type = glsl->get<spirv_cross::SPIRType>(resource.base_type_id);
		samplerTypeString           = glsl->type_to_glsl(type);
		resourceInternalType        = stringTypeToEnum(move(samplerTypeString));

		// Uncapable of detecting combined image samplers (using GL_EXT_nonuniform_qualifier) through decorations, for now the name of the
		// uniform needs to be exactly "textureArrayCombined"
		if ((resource.name == "textureArrayCombined")           &&
		...
		{
			resourceInternalType = ResourceInternalType::RIT_COMBINED_IMAGE_SAMPLER;
		}

		integerFormat               = isIntegerTextureFormat(resourceInternalType);
		sampler                     = new Sampler(move(string(resource.name)), resourceInternalType, shaderStage, binding, set, integerFormat ? VkFilter::VK_FILTER_NEAREST : VkFilter::VK_FILTER_LINEAR);
	...
	for (auto &resource : resources.storage_images)
	{
		...
		samplerTypeString           = glsl->type_to_glsl(type);
		resourceInternalType        = stringTypeToEnum(move(samplerTypeString));
		integerFormat               = isIntegerImageFormat(resourceInternalType);
		sampler                     = new Sampler(move(string(resource.name)), resourceInternalType, shaderStage, binding, set, resourceInternalType, integerFormat ? VkFilter::VK_FILTER_NEAREST : VkFilter::VK_FILTER_LINEAR);
		vecImage.push_back(sampler);
	}

	// Get top level acceleration structure information
	ShaderTopLevelAccelerationStructure* topLevelAccelerationStructure;
	...
	for (auto &resource : resources.acceleration_structures)
	{
		...
		spirv_cross::SPIRType& type   = glsl->get<spirv_cross::SPIRType>(resource.base_type_id);
		samplerTypeString             = glsl->type_to_glsl(type);
		string resName                = string(resource.name);
		resourceInternalType          = stringTypeToEnum(move(samplerTypeString));
		topLevelAccelerationStructure = new ShaderTopLevelAccelerationStructure(move(string(resource.name)), shaderStage, binding, set, move(string(resource.name)), move(string("")));
		vectorTLAS.push_back(topLevelAccelerationStructure);
	}

	const spirv_cross::SmallVector<spirv_cross::Variant>& arrayIds = glsl->getIds();
	const std::unordered_map<spirv_cross::ID, spirv_cross::Meta>& arrayMetaInformation = glsl->getMetaInformation();
	...
	for (auto &resource : resources.uniform_buffers)
	{
		...
		spirv_cross::SPIRType* finalType = nullptr;
		structVariableNameString = "";
		forI(arrayIds.size())
		{
			if (arrayIds[i].get_type() == spirv_cross::TypeVariable)
			{
				auto &var = arrayIds[i].get<spirv_cross::SPIRVariable>();
				finalType = &glsl->get<spirv_cross::SPIRType>(var.basetype);
				if (glsl->type_to_glsl(*finalType, var.self) == resource.name)
				{
					...
					structVariableNameString = arrayMetaInformation.find(var.self)->second.decoration.alias;
					break;
				}
			}
		}
		...
		{
			structFieldNameString                   = arrayMetaInformation.find(finalType->self)->second.members[counter].alias;

			spirv_cross::SPIRType &structMemberType = glsl->get<spirv_cross::SPIRType>(elem);
	...
	for (auto &resource : resources.push_constant_buffers)
	{
		...
		for (const uint32_t& elem : finalType->member_types)
		{
			structFieldNameString    = arrayMetaInformation.find(finalType->self)->second.decoration.alias;
			structVariableNameString = arrayMetaInformation.find(finalType->self)->second.members[counter].alias;
			...
			if (isDataType(structFieldType))
			{
				result = makeUniform(structFieldType, shaderStage, move(string(string(structVariableNameString))), move(string(string(resource.name))), move(string(structFieldNameString)), shaderStruct);
...
bool ShaderReflection::isIntegerTextureFormat(ResourceInternalType resourceInternalType)
{
...
bool ShaderReflection::isIntegerImageFormat(ResourceInternalType resourceInternalType)
{
	switch (resourceInternalType)
	{
		case ResourceInternalType::RIT_INT_IMAGE_1D:
...
{
	const spirv_cross::SPIREntryPoint& entryPoint = compiler.get_entry_point();
